...
const ProfilePage = () => {
    ...
    const handleDownloadTaskData = async (task) => {
        ...
        const userMap = users.reduce((acc, user) => {
            acc[user._id] = { name: user.name, idNumber: user.idNumber };
            return acc;
        }, {});
        
        if (task.type === 'Text cataloging') {
            csvContent.push('User Name,ID Number,Text,Label');
        
            for (const solution of task.solutions) {
                const user = userMap[solution.userID];
                for (const label of solution.solution) {
                    csvContent.push(`${user.name},${user.idNumber},"${task.text}",${label}`);
                }
            }
            ...
            csvContent.push('Image Name,User Name,ID Number,Label');
                    ...
                    if (item.image_url) {
                        try {
                            ...
                            if (response.ok) {
                                ...
                                csvContent.push(`${imageName},${user.name},${user.idNumber},${label}`);
                            } else {
            ...
            for (const solution of task.solutions) {
                ...
                for (const [index, item] of solution.solution.entries()) {
                    ...
                    imagesData.push({ imageName, userName: user.name, idNumber: user.idNumber, label: item.label });
                }
            }
        
            imagesData.sort((a, b) => a.imageName.localeCompare(b.imageName));
        
            imagesData.forEach(({ imageName, userName, idNumber, label }) => {
                csvContent.push(`${imageName},${userName},${idNumber},${label}`);
            });
        
            const csvHeader = 'Image Name,User Name,ID Number,Label\n';
